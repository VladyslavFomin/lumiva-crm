...
export const UtmsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchUtmTemplates()
      ...
      .catch((e: any) => {
        console.error(e);
        setError(e?.message || 'Не удалось загрузить UTM-шаблоны');
      })
      ...
      <div className="space-y-4 md:space-y-6 pb-8">
        <section className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-2xl">
              Быстро собирайте UTM-ссылки для кампаний. Можно сохранять
              пресеты по каналам (Google Ads, Meta Ads, Яндекс, email и т.д.).
            </p>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
          ...
          <div className="rounded-3xl border border-slate-800/80 bg-slate-950/80 px-4 py-4 md:px-5 md:py-5 space-y-4">
            <div>
              <div className="text-[11px] text-slate-400 mb-1">Канал</div>
            ...
            <div className="mt-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3 border-t border-slate-800/80 pt-3">
              <div className="flex-1">
                ...
                <input
                  ...
                  placeholder="Google Brand · RU · весна 2026"
                />
              </div>
              <button
                type="button"
          ...
          <div className="rounded-3xl border border-slate-800/80 bg-slate-950/80 px-4 py-4 md:px-5 md:py-5 text-xs">
            <div className="mb-3 flex items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-slate-500">
                  Хранится по компании (tenant). Можно быстро подставлять
                  параметры для повторяющихся кампаний.
                </p>
              </div>
            ...
            {!loading && !templates.length && (
              <div className="text-[11px] text-slate-500">
                Пока нет сохранённых шаблонов. Сконструируйте UTM-ссылку и
