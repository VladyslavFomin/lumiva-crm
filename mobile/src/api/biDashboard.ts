...
export async function fetchBiDashboardSummary(days?: number): Promise<BiDashboardSummary> {
  const res = await api.get<BiDashboardSummary>('/bi-dashboard/summary', { params: days ? { days } : undefined });
  return res.data;
}
