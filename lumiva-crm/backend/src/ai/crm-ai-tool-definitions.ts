...
/**
 * Инструменты личного дашборда пользователя (главная страница /dashboard — персональная сетка
 * виджетов, НЕ /bi). Каталог id/slug ниже — точное зеркало frontend/src/dashboard/dashboardLayout.ts
 * (DASHBOARD_CORE_WIDGET_IDS/DASHBOARD_EXTRA_WIDGET_IDS) и presetCatalog.ts (PROJECTS_STYLE_PRESETS) —
 * держать в синхроне при изменении набора виджетов там.
 */
export const CRM_DASHBOARD_AI_TOOL_DEFINITIONS: unknown[] = [
  {
    type: 'function',
    function: {
      name: 'crm_dashboard_configure',
      description:
        'Собрать/дополнить/сократить структуру личного дашборда пользователя (главная страница /dashboard — персональная сетка виджетов из аналитики Лидов/Проектов/Продаж/Бронирований/Отелей/Товаров/рабочей области; НЕ путать с /bi — фиксированной общей аналитической сводкой, у неё нет настраиваемой структуры). ' +
        'action:"replace" — полностью пересобрать дашборд ровно из переданного списка widgets (всё остальное скрывается) — используй, когда пользователь описывает желаемую структуру целиком ("собери дашборд с X, Y, Z", "хочу видеть только..."). ' +
        'action:"add" — добавить widgets к уже существующей структуре, ничего не убирая. ' +
        'action:"remove" — убрать виджеты по id из remove (core/extra — их строковый id, например "calendar" или "bookings-analytics"; гранулярный пресет — pid, который был возвращён в предыдущем вызове этого инструмента в этом же диалоге). Если пользователь просит убрать пресет, а его pid неизвестен (не был показан в этом диалоге) — не угадывай pid, используй вместо этого action:"replace" с новым полным списком без этого пресета. ' +
        'ВАЖНО — добавляй ТОЛЬКО то, что явно соответствует просьбе пользователя. Не добавляй виджеты других модулей "заодно" или "на всякий случай" (реальная ошибка прошлого вызова: пользователь попросил лиды/проекты/данные рабочей области, а инструмент без просьбы добавил ещё и аналитику продаж) — если что-то из просьбы нельзя выполнить (см. ниже), явно скажи об этом в ответе, а не заменяй это другим виджетом. ' +
        'Каждый widget одного из трёх видов. ' +
        '(1) {kind:"core", id} — стандартный блок дашборда: kpi, profile-completion, quick-actions, calendar, activity-feed, learn-inspire, leads-timeline, projects, channels-funnel, recent-leads, recent-tasks, staff. ' +
        '(2) {kind:"extra", id} — ЦЕЛЫЙ встроенный блок-сводка аналитики модуля: projects-analytics, leads-analytics, sales-analytics, products-analytics, bookings-analytics, hotels-analytics. Это готовая мини-страница с 2 фиксированными метриками — используй ТОЛЬКО когда пользователь хочет общую сводку по всему модулю Бронирований/Отелей/Товаров (у них нет других вариантов, разбивки на метрики не существует). НЕ используй extra, если пользователь просит "таблицу"/"список"/"график"/"по статусам"/"по категориям" по Лидам/Проектам/Продажам — для этого есть (3) preset, extra в этих случаях не даст ни таблицы, ни графика, только 2 общих числа (реальная ошибка прошлого вызова: на запрос "таблица проектов" был добавлен extra projects-analytics вместо preset table-projects). ' +
        '(3) {kind:"preset", source, slug} — ОДНА конкретная метрика/график/таблица. source: "leads"|"projects"|"sales" — стандартные модули; "workspace" — таблица рабочей области /workspace (ЛЮБЫЕ данные, которые пользователь туда импортировал или создал сам — включая выгруженную рекламу Google Ads/Meta/Яндекс.Директ через crm_workspace_import_marketing_channels/_monthly_breakdown, кастомные таблицы и т.п.); для source:"workspace" ОБЯЗАТЕЛЕН sourceRef — реальный objectId таблицы, который нужно сначала узнать через crm_workspace_list_tables (найти таблицу по названию, которое назвал пользователь) — НИКОГДА не выдумывай sourceRef. slug — одно из: metric-total (кол-во записей), metric-amount (сумма поля amount/сумма), metric-owners (кол-во уникальных ответственных), metric-won (кол-во выигранных/забронированных — неприменимо к workspace), chart-status (донат по статусам), chart-categories (столбцы по категориям/источникам), table-projects (таблица всех записей — используй именно этот slug, когда пользователь просит "таблицу"/"список"/"все записи"). Для таблиц рабочей области (например Google Ads) осмысленны обычно metric-total, table-projects и chart-categories (по колонке-категории, если такая есть в таблице) — chart-status/metric-won могут быть неприменимы, если в таблице нет статуса/поля "выиграно"; если не уверен, что в таблице есть подходящее поле — сначала проверь crm_workspace_describe_table и не добавляй заведомо пустой/бессмысленный виджет. ' +
        'БОГАТСТВО ОТОБРАЖЕНИЯ: пользователь хочет не только цифры, а наглядный дашборд — по умолчанию НЕ ограничивайся одним bare-metric виджетом на тему. На каждую запрошенную тему добавляй небольшой осмысленный набор: metric (число) + один график (chart-status ИЛИ chart-categories — какой уместнее для этих данных) + table-projects, если пользователь упомянул таблицу/список/"все записи". Например "аналитика по лидам" → metric-total (или metric-amount) + chart-status или chart-categories для лидов, а не только один metric. Если пользователь попросил ровно одну конкретную вещь ("только количество лидов, больше ничего") — тогда именно её и одну, не добавляй лишнего. ' +
        'title — необязательная строка, свой заголовок виджета вместо стандартного (у стандартных заголовков пресетов, к сожалению, нет привязки к конкретному источнику — например "leads"+metric-total и "sales"+metric-total выглядят одинаково без title). ОБЯЗАТЕЛЬНО передавай title для source:"workspace" (иначе виджет не будет понятно, какая именно таблица в нём — например title:"Google Ads — сессии/расход"), и рекомендуется передавать title во всех остальных случаях, где на дашборде может быть больше одного похожего виджета. ' +
        'filters:{dateFrom,dateTo} (YYYY-MM-DD) — необязательный период ("за последний месяц" и т.п.); без него виджет считает за весь период. size — необязательно, sm/md/lg. ' +
        'После вызова в ответе перечисли пользователю итоговую структуру дашборда простыми словами (из поля structure ответа) — если что-то из просьбы не попало в дашборд (например неподходящий slug для workspace-таблицы), явно скажи, что именно и почему. Напомни, что изменения появятся на /dashboard (обновить страницу, если уже открыта).',
      parameters: {
        type: 'object',
        properties: {
          action: { type: 'string', enum: ['replace', 'add', 'remove'] },
          widgets: {
            type: 'array',
            description: 'Виджеты для action "replace" (полный новый список) или "add" (только новые). Не используется для "remove".',
            items: {
              type: 'object',
              properties: {
                kind: { type: 'string', enum: ['core', 'extra', 'preset'] },
                id: { type: 'string', description: 'Для kind core/extra — id виджета из описания инструмента.' },
                source: { type: 'string', enum: ['leads', 'projects', 'sales', 'workspace'], description: 'Только для kind:"preset".' },
                sourceRef: { type: 'string', description: 'Только для source:"workspace" — реальный objectId таблицы из crm_workspace_list_tables. Обязателен для workspace.' },
                slug: { type: 'string', description: 'Только для kind:"preset" — одно из значений из описания инструмента.' },
                title: { type: 'string', description: 'Свой заголовок виджета. Обязателен для source:"workspace", рекомендуется в остальных случаях при риске неоднозначности.' },
                size: { type: 'string', enum: ['sm', 'md', 'lg'] },
                filters: {
                  type: 'object',
                  description: 'Только для kind:"preset" — необязательный период.',
                  properties: {
          ...
          remove: {
            type: 'array',
            items: { type: 'string' },
            description: 'Только для action:"remove" — id виджетов (core/extra id или pid_ пресета из предыдущего ответа этого инструмента).',
          },
        },
        required: ['action'],
      },
