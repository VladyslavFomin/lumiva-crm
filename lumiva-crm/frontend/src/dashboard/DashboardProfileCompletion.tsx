    ...
    return (
      <>
        <div className="flex flex-col gap-px">
          {presentSteps.map((id) => {
            ...
            return (
              <div
                key={id}
                className={`flex items-center gap-2.5 px-1 py-[7px] rounded-md cursor-pointer transition-colors hover:bg-neutral-50 text-[12.5px] ${done ? 'text-neutral-500' : 'text-[#222]'}`}
              >
                <span
                ...
                <span className={`flex-1 leading-snug ${done ? 'line-through decoration-neutral-300 decoration-[1px]' : ''}`}>
                  {t(`crm.dashboard.profileCompletion.steps.${id}.label`)}
                </span>
                <svg className={`w-2.5 h-2.5 text-neutral-300 transition-opacity ${done ? 'opacity-0' : 'opacity-100'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M9 6l6 6-6 6" />
                </svg>
              </div>
            );
        ...
        <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-neutral-100">
          ...
          <Link
            to="/leads/new"
            className="inline-flex items-center justify-center rounded-[7px] border border-neutral-200 bg-white px-3 py-1.5 text-[11px] font-medium text-[#222] hover:border-neutral-300 transition-colors"
          >
    ...
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <div
            className="mt-1 text-2xl font-semibold text-[#222] tabular-nums"
          >
            {percent}%
          </div>
        </div>
        <div className="h-2 w-32 max-w-[40%] rounded-full bg-neutral-100 overflow-hidden">
          <div
          ...
          return (
            <li
              ...
              className={`flex items-start gap-2.5 rounded-2xl border px-3 py-2.5 text-[12px] ${
                done ? 'border-emerald-200/80 bg-emerald-50/50 text-neutral-700' : 'border-neutral-200 bg-white text-[#222]'
              }`}
            >
              <span
                className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${
                  done ? 'bg-emerald-500 text-white' : 'border border-neutral-300 text-neutral-400'
                }`}
                aria-hidden
              >
                {done ? '✓' : ''}
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        ...
        <Link
          to="/leads/new"
          className="inline-flex items-center justify-center rounded-xl border border-neutral-200 bg-white px-3 py-2 text-[11px] font-medium text-[#222] hover:border-neutral-300 transition-colors"
        >
          {t('crm.dashboard.profileCompletion.createLead')}
        </Link>
      </div>
