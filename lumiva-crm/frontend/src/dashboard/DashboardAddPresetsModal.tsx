...
export const DashboardAddPresetsModal: React.FC<{
  ...
  useEffect(() => {
    ...
    loadAnalyticsItemsForSource(tab).then((data) => {
  ...
  return (
    <div className="fixed inset-0 z-[10080] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div
        role="dialog"
        className="w-full max-w-5xl max-h-[92vh] overflow-hidden flex flex-col rounded-[28px] border border-slate-200 bg-white shadow-[0_25px_80px_rgba(15,23,42,0.35)] text-slate-900"
      >
        <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-slate-100">
          <div>
            ...
            <h2 className="text-sm font-semibold text-slate-900">{t('crm.dashboard.presets.modalTitle')}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl border border-slate-300 bg-white px-3 py-1.5 text-[11px] font-medium text-slate-800 hover:bg-slate-50"
          >
            {t('crm.common.close')}
          </button>
        </div>

        <div className="px-5 pt-4 flex flex-wrap gap-2 border-b border-slate-50 pb-4">
          {tabs.map((x) => (
            <button
              key={x.id}
              type="button"
        ...
        <div key={tab} className="flex-1 overflow-y-auto px-5 py-4">
          <p className="text-[11px] text-slate-600 mb-4">{t('crm.dashboard.presets.pickHint')}</p>
            ...
            {rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[12px] font-semibold text-slate-900 px-3 pb-2 leading-snug">
                    {title}
                  </div>
                  <div className="h-[200px] border-y border-slate-100 bg-gradient-to-b from-slate-50/80 to-white px-2 py-1">
                    <div className="h-full w-full overflow-hidden rounded-xl bg-white border border-slate-100 shadow-inner px-2 py-2">
          ...
          {hiddenStandardIds.length > 0 && (
            <div className="mt-8 pt-6 border-t border-slate-100">
              <div className="text-[11px] font-semibold text-slate-800 mb-3">
              ...
              <div className="flex flex-wrap gap-2">
                {hiddenStandardIds.map((id) => (
                  <button
                    ...
                    className="rounded-xl border border-dashed border-slate-300 bg-white px-3 py-2 text-[11px] font-medium text-slate-800 hover:bg-slate-50"
                  >
                    + {widgetTitle(id)}
                  </button>
                ))}
