...
export const DashboardLearnInspire: React.FC<Props> = ({ slugs, inSidebar }) => {
      ...
      <div className="flex flex-col gap-px">
        {STATIC_LINKS.map((item) => (
          <Link
            key={item.key}
            to={item.href}
            className="flex items-center gap-2.5 px-1 py-[7px] rounded-md text-[12.5px] text-[#222] hover:bg-neutral-50 transition-colors group"
          >
            <span className="text-neutral-400 flex-shrink-0">{item.icon}</span>
            <span className="flex-1 leading-snug">
              {t(`crm.dashboard.learn.links.${item.i18nKey}`, { defaultValue: item.fallback })}
            </span>
            <svg
              className="w-2.5 h-2.5 text-neutral-300 opacity-0 group-hover:opacity-100 transition-opacity"
        ...
        <div className="pt-2 mt-1 border-t border-neutral-100">
          <Link
            to="/blog"
            className="text-[11px] font-medium text-neutral-500 hover:text-[#222] underline-offset-2 hover:underline transition-colors"
          >
  ...
  return (
    <div className="space-y-3">
      ...
      <ul className="space-y-2">
        {list.map((slug) => (
          <li key={slug}>
            <Link
              to={`/blog#post-${slug}`}
              className="group flex flex-col rounded-2xl border border-neutral-200/90 bg-gradient-to-b from-white to-neutral-50/80 px-3 py-2.5 transition hover:border-sky-300/80 hover:shadow-sm"
            >
              <span className="text-[12px] font-semibold text-[#222] group-hover:text-sky-800">
                {t(`crm.dashboard.learn.posts.${slug}.title`, { defaultValue: slug })}
              </span>
              <span className="text-[11px] text-neutral-500 mt-0.5 line-clamp-2">
      ...
      <Link
        to="/blog"
        className="inline-flex text-[11px] font-medium text-neutral-700 hover:text-[#222] underline-offset-2 hover:underline"
      >
        {t('crm.dashboard.learn.openBlog')}
      </Link>
    </div>
