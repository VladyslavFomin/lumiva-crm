    ...
    <div style={{ width: '100%' }}>
      ...
      {categories.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
          <button
            ...
            style={{ borderRadius: 999, padding: '4px 10px', fontSize: 12, border: `1px solid ${!activeCategory ? accent : border}`, background: !activeCategory ? accent : 'transparent', color: !activeCategory ? '#fff' : d.textColor as string }}
          >
            Все
          </button>
          {categories.map((c) => (
            <button
              ...
              style={{ borderRadius: 999, padding: '4px 10px', fontSize: 12, border: `1px solid ${activeCategory === c.id ? accent : border}`, background: activeCategory === c.id ? accent : 'transparent', color: activeCategory === c.id ? '#fff' : d.textColor as string }}
            >
              {c.name}
            </button>
          ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 10, marginBottom: 12 }}>
        {products.map((p) => {
          ...
          return (
            <div key={p.id} style={{ border: `1px solid ${border}`, borderRadius: radius, padding: 8 }}>
              <div style={{ height: 70, borderRadius: radius - 2, background: String(d.fieldBackground || '#f9fafb'), display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', marginBottom: 6 }}>
              ...
              <div style={{ fontSize: 12, marginBottom: 6 }}>{p.price} {p.currency}</div>
              <button
                type="button"
                onClick={() => addProduct(p)}
                style={{ width: '100%', borderRadius: 999, padding: '4px 0', fontSize: 12, fontWeight: 600, background: accent, color: '#fff', border: 'none' }}
              >
                Добавить
              </button>
            </div>
      ...
      {lines.length > 0 && (
        <div style={{ border: `1px solid ${border}`, borderRadius: radius, padding: 10 }}>
          {lines.map((l) => (
            <div key={l.sku} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, padding: '4px 0' }}>
              ...
              <input
                ...
                style={{ width: 48, border: `1px solid ${border}`, borderRadius: 6, padding: '2px 4px', textAlign: 'center' }}
              />
              <span style={{ width: 70, textAlign: 'right' }}>{(l.price * l.qty).toFixed(2)}</span>
              <button type="button" onClick={() => setQty(l.sku, 0)} style={{ border: 'none', background: 'none', color: '#94a3b8' }}>✕</button>
            </div>
          ))}
          <div style={{ textAlign: 'right', fontWeight: 600, fontSize: 13, marginTop: 6 }}>
