...
export const CRM_EXTENDED_AI_TOOL_DEFINITIONS: unknown[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Создать автоматизацию: name, triggerEvent, actions (массив { type, config }). Для периодических действий используй triggerEvent scheduled и meta: { schedule: { scheduleFrequency: weekly|daily|monthly|quarterly, scheduleTime: HH:mm, scheduleTimezone: IANA, scheduleDayOfWeek: 1-7 (пн=1), scheduleDayOfMonth: 1-31 } }. Действие send_email: accountId, to (массив), templateId или subject/textBody.',
      parameters: {
        type: 'object',
        properties: {
          name: { type: 'string' },
